...
local function main_loop_iter()
    ...
    local rready, wready, err = socket.select(rsockets, wsockets, 1)
    if err and err ~= "timeout" then irc_debug._err(err); return false; end
...
local timerCallbackTime = {}
local TEMPtimerCallbacks = {}
local TEMPtimerCallbackTime = {}
local timerLocked = false
function registerTimerCallback(duration, func, chan)
	if timerLocked then
		TEMPtimerCallbackTime[chan] = socket.gettime() + duration
		TEMPtimerCallbacks[chan] = func
	else
		timerCallbackTime[chan] = socket.gettime() + duration
		timerCallbacks[chan] = func
	end
end

-- begin_main_loop {{{
local function begin_main_loop()
	while main_loop_iter() do
		timerLocked = true
		local current_time = socket.gettime()
		for chan, callbackTime in base.pairs(timerCallbackTime) do
			if current_time > callbackTime then
				timerCallbacks[chan](chan)
				timerCallbackTime[chan] = nil
				timerCallbacks[chan] = nil
			end
		end
		timerLocked = false
		for chan, callbackTime in base.pairs(TEMPtimerCallbackTime) do
			timerCallbackTime[chan] = TEMPtimerCallbackTime[chan]
			timerCallbacks[chan] = TEMPtimerCallbacks[chan]
			TEMPtimerCallbackTime[chan] = nil
...
--
function handlers.on_err_nicknameinuse()
	send("NICK", argsSaved.nick.."_")
end
...
function handlers.on_rpl_welcome(from)
	...
	local mt = {
		...
		__newindex = function(self, key, value)
			base.rawset(self, key:lower(), value)
		end
	}
	base.setmetatable(serverinfo.channels, mt)
end
-- }}}
...
-- on_rpl_endofmotd {{{
function handlers.on_err_nomotd(from)
    handlers.on_rpl_endofmotd(from)
end
-- }}}
